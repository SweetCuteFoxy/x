...
## 2. .NET проект: создание

```powershell
cd ShopApp
dotnet new winforms -n ShopApp -f net9.0-windows --force
...
```
Внутри `<ItemGroup>` для папки фото:
```xml
<Content Include="photos\**\*"><CopyToOutputDirectory>PreserveNewest</CopyToOutputDirectory></Content>
```
